...
class SEBIFolderPipelineParser:
    ...
    def extract_legal_metadata(self, markdown_text: str) -> dict:
        ...
        circ_match = re.search(r'(?:SEBI/|HO/)[a-zA-Z0-9_/: \t\(\)-]+', first_page_chunk)
    ...
    async def parse_document_async(self, pdf_path: Path):
        ...
        try:
            ...
            if os.path.exists(md_path):
                print(f"💾 Found local cache for '{filename}'. Bypassing Cloud API call.")
...
async def main():
    ...
    if not api_key:
        print("CRITICAL ERROR: Please export LLAMA_CLOUD_API_KEY before running this pipeline script.")
        return
